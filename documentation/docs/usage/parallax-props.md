...
| **rootMargin**                       |        `object`        |         | Margin to be applied as the bounds around an element. This will affect when an element is determined to be in the viewport. Example: `{ top: 100, right: 100, bottom: 100, left: 100 }`. N.B. You must provide all fields of the rootMargin object for this to work properly, even if those values are 0. Example:</br>✅ `{ top: 0, right: 0, bottom: 100, left: 0 }`, </br>❌ `{ bottom: 100 }`.                                                                                                                    |
