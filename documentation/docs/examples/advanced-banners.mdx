...
The `<ParallaxBanner>` accepts advanced configurations that can allow you to really push the effect. Building off the [previous banner demos](/docs/v4/examples/banners), this one adds a number of configuration options.
...
const Component = () => {
  const background: BannerLayer = {
    ...
    translateY: [0, 50],
    opacity: [1, 0.3],
    scale: [1.05, 1, 'cubic-bezier(0.215, 0.61, 0.355, 1)'],
    shouldAlwaysCompleteAnimation: true,
  };

  const headline: BannerLayer = {
    translateY: [0, 30],
    scale: [1, 1.05, 'cubic-bezier(0.215, 0.61, 0.355, 1)'],
    shouldAlwaysCompleteAnimation: true,
    expanded: false,
    children: (
  ...
  const foreground: BannerLayer = {
    ...
    translateY: [0, 15],
    scale: [1, 1.1, 'cubic-bezier(0.215, 0.61, 0.355, 1)'],
    shouldAlwaysCompleteAnimation: true,
  };
...
Each layer of the `<ParallaxBanner>` is just a `useParallax` hook targeting a `<div>`. Which means anything you can use to configure [`useParallax`](/docs/v4/usage/hooks/use-parallax) can be used as a property of a `layer`. See all [effects and configuration](/docs/v4/usage/parallax-props) accepted.
