...
This repository is a **pnpm + Turborepo** monorepo. From the repo root:

- `pnpm build` — build all packages and apps
- `pnpm test` — run library tests
- `pnpm storybook` — Storybook (`apps/storybook`)
- `pnpm docs` — Docusaurus (`apps/docs`)

The publishable React library lives in `packages/react-scroll-parallax` (run `pnpm start` there for `tsdown --watch`).

Code formatting uses **Prettier** (see root `package.json`).

### Vitest

Vitest tests live under `packages/react-scroll-parallax` and `packages/scroll-parallax`; run `pnpm test` from the monorepo root.
