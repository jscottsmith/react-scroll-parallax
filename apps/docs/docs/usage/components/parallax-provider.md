...
The `<ParallaxProvider />` component is meant to wrap a root level component in your application and is necessary to provide access through the React context API to the [scroll-parallax](/docs/scroll-parallax/intro).
...
You can have multiple providers, however they will all be independent instances of a [`ParallaxController`](/docs/scroll-parallax/api/parallax-controller/). It's recommended to only use one when possible.
