...
React hooks and components to create parallax scroll effects for banners, images or any other DOM elements. Utilizes [scroll-parallax](/docs/scroll-parallax/intro) to add vertical or horizontal scrolling based effects to elements. [Optimized](/docs/scroll-parallax/performance) to _reduce_ jank on scroll and works with SSR and SSG rendered React apps.
